...
def main():
    ...
    k_lab=[]
    for k in range(1,50):
        print('Testing for ',k,' iteration(s)')
        conf_matrix,accuracy=load.cross_val(nXtrain,nXtest,ytrain,ytest,'k_means',k) 
        k_lab.append(k)  
        n_conf_matrix_arr.append(conf_matrix)
        n_accuracy_arr.append(accuracy)
        
        conf_matrix,accuracy=load.cross_val(pXtrain,pXtest,ytrain,ytest,'k_means',k) 
        p_conf_matrix_arr.append(conf_matrix)
        p_accuracy_arr.append(accuracy)
        
        conf_matrix,accuracy=load.cross_val(fXtrain,fXtest,ytrain,ytest,'k_means',k) 
    ...
    plt.ylabel('accuracy')
    plt.legend()
    plt.savefig('k_means_iteration_acc.png')
    plt.show()
